...
void addnoise(Cloud &cloud_a){

	Cloud::Ptr cloudfiltered(new Cloud);
	cloudfiltered->points.resize(cloud_a.size());

	boost::mt19937 rng;
	rng.seed(static_cast<unsigned int> (time(0)));
	boost::normal_distribution<> nd(1,0.15);
	boost::variate_generator<boost::mt19937&, boost::normal_distribution<> > var_nor(rng, nd);

	for (size_t point_i = 0; point_i < cloud_a.size(); ++point_i)
	{
		cloudfiltered->points[point_i].x = cloud_a.points[point_i].x + static_cast<float> (var_nor());
		cloudfiltered->points[point_i].y = cloud_a.points[point_i].y + static_cast<float> (var_nor());
		cloudfiltered->points[point_i].z = cloud_a.points[point_i].z + static_cast<float> (var_nor());
	}
	pcl::io::savePLYFileASCII("F:/Projects/compare_cloud/compare_cloud/data/block_noise0223.ply", *cloudfiltered);
	std::cout << "add noise " << endl;
}
...
void compute(Cloud &cloud_a, Cloud &cloud_b)
{
	...
	for (size_t i = 0; i < cloud_a.points.size(); ++i)
	{
		...
		dist_AB.push_back(sqr_distances[0]);
	...
	pcl::io::savePLYFileASCII("F:/Projects/compare_cloud/compare_cloud/data/cloud_new.ply", cloud_i);
...
int main(int argc, char** argv){
	...
	pcl::io::loadPLYFile("F:/Projects/compare_cloud/compare_cloud/data/block2.ply", *cloud_a);

	addnoise(*cloud_a);

	Cloud::Ptr cloud_b(new Cloud);
	//pcl::io::loadPLYFile("F:/Projects/compare_cloud/compare_cloud/data/bunnybf1.ply",*cloud_b);
	pcl::io::loadPLYFile("F:/Projects/compare_cloud/compare_cloud/data/block_noise0222.ply", *cloud_b);
	...
	pcl::io::loadPLYFile("F:/Projects/compare_cloud/compare_cloud/data/cloud_new.ply", *cloud_i);
...
// 		cloud_out->points[i].z = cloud_i->points[i].z;
	...
	//pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZI> fildColor(cloud_i, "i"); // °´ÕÕz×Ö¶Î½øÐÐäÖÈ¾
